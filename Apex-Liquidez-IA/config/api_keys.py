...
POLYGON_API_KEY = os.getenv("POLYGON_API_KEY", "COLE_AQUI")

# Alpha Vantage — dados históricos e fallback
# https://alphavantage.co → Get free API key
ALPHA_VANTAGE_API_KEY = os.getenv("ALPHA_VANTAGE_API_KEY", "COLE_AQUI")

# Binance — dados de criptomoedas (BTC, ETH)
# https://binance.com → Account → API Management
BINANCE_API_KEY = os.getenv("BINANCE_API_KEY", "COLE_AQUI")
BINANCE_SECRET_KEY = os.getenv("BINANCE_SECRET_KEY", "COLE_AQUI")

# NewsAPI — notícias financeiras para sentimento
# https://newsapi.org → Get API Key
NEWS_API_KEY = os.getenv("NEWS_API_KEY", "COLE_AQUI")
...
# ============================================================
MT5_BRIDGE_URL = os.getenv("MT5_BRIDGE_URL", "http://localhost:5001")
MT5_BRIDGE_TOKEN = os.getenv("MT5_BRIDGE_TOKEN", "apex_local_token_2024")
...
MAX_DRAWDOWN = float(os.getenv("MAX_DRAWDOWN", "8.0"))

# Intervalo do loop principal (segundos)
LOOP_INTERVAL = int(os.getenv("LOOP_INTERVAL", "15"))

# Score mínimo para gerar sinal (0-10)
MIN_SCORE_BUY  =  int(os.getenv("MIN_SCORE_BUY", "4"))
MIN_SCORE_SELL = int(os.getenv("MIN_SCORE_SELL", "-4"))
